...
function assignVacationRows(vacationsByDate, month) {
    const vacationRows = {};
    ...
    employees.forEach(employee => {
        employee.vacations.forEach(vacation => {
            ...
            const workingDates = getDateRange(startDate, endDate)
                .filter(date => isWorkingDay(date))
                .filter(date => {
                    if (month === 7) {
                        return (date.getMonth() === 5 && date.getDate() === 30) || 
                               (date.getMonth() === 6 && date.getFullYear() === 2025);
                    } else if (month === 8) {
    ...
    // For August, handle special positioning rules
    if (month === 8) {
        return assignAugustVacationRows(continuousVacations, vacationsByDate);
    }
    
    // For July, use standard assignment
    return assignStandardVacationRows(continuousVacations);
}

function assignAugustVacationRows(continuousVacations, vacationsByDate) {
    const vacationRows = {};
    const dateRows = {};
    ...
    Object.keys(augustPositionRules).forEach(dateKey => {
        ...
        Object.keys(rules).forEach(employeeName => {
            const rowIndex = rules[employeeName];
            const vacationKey = Object.keys(continuousVacations).find(key => 
                continuousVacations[key].employee.name === employeeName && 
                continuousVacations[key].dates.some(date => formatDate(date) === dateKey)
            );
            
            if (vacationKey) {
                vacationRows[vacationKey] = rowIndex;
                
                // Reserve this row for all dates in the continuous vacation
                continuousVacations[vacationKey].dates.forEach(date => {
                    const dateStr = formatDate(date);
                    if (!dateRows[dateStr]) {
                        dateRows[dateStr] = [];
                    }
                    dateRows[dateStr][rowIndex] = vacationKey;
                });
            }
        });
    });
    
    // Then assign remaining vacations
...
function assignStandardVacationRows(continuousVacations) {
    const vacationRows = {};
    const dateRows = {};
...
function createVacationBar(container, vacationData, rowIndex) {
    ...
    bar.style.top = `${25 + rowIndex * 25}px`;
    bar.style.left = '2px';
    bar.style.right = '2px';
